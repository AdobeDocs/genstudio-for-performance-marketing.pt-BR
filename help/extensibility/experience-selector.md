...
> Serviço de API disponível> A API do GenStudio oferece uma API RESTful para gerenciar experiências. Se você quiser usar a API do GenStudio para gerenciar experiências, consulte a [documentação da API do GenStudio](https://developer.adobe.com/genstudio-api/).
...
Para incorporar este MFE ao Salesforce Lightning com LWC e modelos de email opcionais, consulte [MFE do Seletor de Experiência no Salesforce](experience-selector-salesforce.md).
...
| `imsOrg` | string | Sim | ID da organização IMS |
...
## Retorno de chamada de seleção

Quando um [!DNL Experience] é selecionado no MFE e o botão **[!UICONTROL Usar]** é clicado, uma função de retorno de chamada deve ser invocada para passar os detalhes do [!DNL Experience] selecionado.

O retorno de chamada fornece uma cadeia de caracteres HTML representando o [!DNL Experience]. Se a Experiência incluir ativos (por exemplo, imagens, vídeos), os elementos da HTML, como `<img>` e `<video>`, incluirão `src` atributos que contêm URLs pré-assinadas para esses ativos. Essas URLs pré-assinadas têm um tempo de expiração de 30 minutos, e o [!DNL Experience] deve ser consumido pelo aplicativo cliente nesse tempo.

### Esquema de saída

O retorno de chamada `onSelectionConfirmed` recebe uma matriz de objetos [!DNL Experience] com a seguinte estrutura:

| Propriedade | Tipo | Descrição |
|----------|------|-------------|
| `id` | `string` | Identificador exclusivo do [!DNL Experience] (formato URN.) |
| `content` | `string` | Cadeia de caracteres HTML codificada na Base64 representando o [!DNL Experience]. |
| `experienceFields` | `object` | Pares de valores-chave de campos editáveis no [!DNL Experience]. Cada campo em `experienceFields` contém: <ul><li>`fieldName`: O nome/identificador do campo.</li><li>`fieldValue`: o conteúdo/valor real do campo.</li></ul> |
| `metadata` | `object` | Metadados sobre a experiência (por exemplo, `channel`, `externalTemplateMetadata`, `externalAssetMetadata` etc.) |
| `aspectVariants` | `array` | Matriz de variantes de taxa de proporção para [!DNL Experience]. Cada variante contém: <ul><li>`aspectKey`: identificador exclusivo da variante.</li><li>`aspectMetadata`: Metadados, incluindo `channel` e `aspectRatio`.</li><li>`content`: Cadeia de caracteres HTML codificada em Base64 para esta taxa de proporção específica.</li></ul> |

### Exemplo de JSON

Abaixo está uma amostra do bloco JSON que mostra os esquemas.
